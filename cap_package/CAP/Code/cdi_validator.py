...
def CDI_Masterlist_QA(cdi_dataset):
    ...
    change_dict["name"] = Invalid_Updated_toDict(
        name_change
    )  # Index 0 correlates to Name
    change_dict["title"] = Invalid_Updated_toDict(title_change)
    change_dict["organization"] = Invalid_Updated_toDict(org_change)
    change_dict["catalog_url"] = Invalid_Updated_toDict(
        catalog_change
    )  # Index 1 correlates to cat url
...
def Invalid_Updated_toDict(listof_invalid_updated):
    ...
    invalid_updates_dict = {"Invalid": before, "Updated": after}
...
def Extra_Data_Gov(masterlist_json, date):
    ...
    api_call = requests.get(
        "https://catalog.data.gov/api/3/action/package_search?fq=groups:climate5434&rows=2000"
    ).json()

    # with open("climate_collection.json", "r") as climatefile:
    #     api_call = json.load(climatefile)

    data_gov_id_df = pd.json_normalize(
        api_call["result"]["results"]
    )  # Create a Dataframe of all Data.gov IDs in Data.gov CC
    ...
    for index, row in data_gov_id_df.iterrows():
        ...
        if row["id"] in masterlist_id_list:
            ...
            row[
                "API"
            ] = "https://catalog.data.gov/api/3/action/package_show?id={}".format(
                row["id"]
            )
    ...
    formatted_df = pd.DataFrame(
        {
            "date_id": date,
            "title": not_in_master_full["title"],
            "name": not_in_master_full["name"],
            "api_url": not_in_master_full["API"],
            "catalog_url": not_in_master_full["Catalog"],
        }
    )
